...
// Item stored in Server's offline pool (generated by third-party Dealer)
struct ServerPoolItem {
  uint64_t dpf_id;                  // Unique sync ID assigned by dealer
  uint64_t r_share;                 // XOR share of the random dummy index r (r = r0 ^ r1)
  std::vector<bool> expanded_vector; // Pre-expanded boolean vector for O(1) online evaluation
};
